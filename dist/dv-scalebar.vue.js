import { defineComponent as Tt, h as I } from "vue";
/**
 * @license
...
const Xe = (...e) => {
  ...
  const { kE: r, kK: o, kKE: s, Xn: c, Yn: f, Zn: a } = K, i = (e + 16) / 116, b = 2e-3 * t + i, d = i - 5e-3 * n, h = b * b * b, g = d * d * d, _ = h > r ? h : (116 * b - 16) / o, C = e > s ? Math.pow((e + 16) / 116, 3) : e / o, y = g > r ? g : (116 * d - 16) / o, m = _ * c, S = C * f, P = y * a;
  return [m, S, P];
}, $e = (e) => {
  const t = Math.sign(e);
  return e = Math.abs(e), (e <= 31308e-7 ? e * 12.92 : 1.055 * Math.pow(e, 1 / 2.4) - 0.055) * t;
}, pt = (e, t, n) => {
  const { MtxAdaptMa: r, MtxAdaptMaI: o, MtxXYZ2RGB: s, RefWhiteRGB: c, Xn: f, Yn: a, Zn: i } = K, b = f * r.m00 + a * r.m10 + i * r.m20, d = f * r.m01 + a * r.m11 + i * r.m21, h = f * r.m02 + a * r.m12 + i * r.m22, g = c.X * r.m00 + c.Y * r.m10 + c.Z * r.m20, _ = c.X * r.m01 + c.Y * r.m11 + c.Z * r.m21, C = c.X * r.m02 + c.Y * r.m12 + c.Z * r.m22, y = (e * r.m00 + t * r.m10 + n * r.m20) * (g / b), m = (e * r.m01 + t * r.m11 + n * r.m21) * (_ / d), S = (e * r.m02 + t * r.m12 + n * r.m22) * (C / h), P = y * o.m00 + m * o.m10 + S * o.m20, j = y * o.m01 + m * o.m11 + S * o.m21, $ = y * o.m02 + m * o.m12 + S * o.m22, x = $e(
...
function Jt(e, t, n) {
  const { Xn: r, Yn: o, Zn: s, kE: c, kK: f } = K, a = e / r, i = t / o, b = n / s, d = a > c ? Math.pow(a, 1 / 3) : (f * a + 16) / 116, h = i > c ? Math.pow(i, 1 / 3) : (f * i + 16) / 116, g = b > c ? Math.pow(b, 1 / 3) : (f * b + 16) / 116;
...
const mt = (e, t, n) => {
  ...
  const { MtxRGB2XYZ: r, MtxAdaptMa: o, MtxAdaptMaI: s, Xn: c, Yn: f, Zn: a, As: i, Bs: b, Cs: d } = K;
  let h = e * r.m00 + t * r.m10 + n * r.m20, g = e * r.m01 + t * r.m11 + n * r.m21, _ = e * r.m02 + t * r.m12 + n * r.m22;
  const C = c * o.m00 + f * o.m10 + a * o.m20, y = c * o.m01 + f * o.m11 + a * o.m21, m = c * o.m02 + f * o.m12 + a * o.m22;
  let S = h * o.m00 + g * o.m10 + _ * o.m20, P = h * o.m01 + g * o.m11 + _ * o.m21, j = h * o.m02 + g * o.m12 + _ * o.m22;
  return S *= C / i, P *= y / b, j *= m / d, h = S * s.m00 + P * s.m10 + j * s.m20, g = S * s.m01 + P * s.m11 + j * s.m21, _ = S * s.m02 + P * s.m12 + j * s.m22, [h, g, _];
};
...
u.prototype.darken = function(e = 1) {
  ...
  return n[0] -= K.Kn * e, new u(n, "lab").alpha(t.alpha(), !0);
};
...
const { sin: on, cos: sn } = Math, gt = (...e) => {
  ...
  const [t, n, r, ...o] = M(e, "rgb"), [s, c, f] = qe(t, n, r), [a, i, b] = yt(s, c, f);
  return [a, i, b, ...o.length > 0 && o[0] < 1 ? [o[0]] : []];
};
...
u.prototype.saturate = function(e = 1) {
  ...
  return n[1] += K.Kn * e, n[1] < 0 && (n[1] = 0), new u(n, "lch").alpha(t.alpha(), !0);
};
...
const ce = (e, t, n, r) => {
  ...
  let h, g, _, C;
  return !isNaN(c) && !isNaN(f) ? (f > c && f - c > 180 ? C = f - (c + 360) : f < c && c - f > 180 ? C = f + 360 - c : C = f - c, g = c + n * C) : isNaN(c) ? isNaN(f) ? g = Number.NaN : (g = f, (b == 1 || b == 0) && r != "hsv" && (h = i)) : (g = c, (d == 1 || d == 0) && r != "hsv" && (h = a)), h === void 0 && (h = a + n * (i - a)), _ = b + n * (d - b), r === "oklch" ? new u([_, h, g], r) : new u([g, h, _], r);
...
const Ie = (...e) => {
  e = M(e, "lab");
  const [t, n, r, ...o] = e, [s, c, f] = qn([t, n, r]), [a, i, b] = pt(s, c, f);
  return [a, i, b, ...o.length > 0 && o[0] < 1 ? [o[0]] : []];
};
function qn(e) {
  var t = [
...
const Ke = (...e) => {
  ...
  return [...Tn(s), ...o.length > 0 && o[0] < 1 ? [o[0]] : []];
};
function Tn(e) {
  const t = [
...
function ye(e) {
  let t = "rgb", n = E("#ccc"), r = 0, o = [0, 1], s = [], c = [0, 0], f = !1, a = [], i = !1, b = 0, d = 1, h = !1, g = {}, _ = !0, C = 1;
  const y = function(l) {
  ...
  const j = function(l, p) {
    ...
    if (p == null && (p = !1), isNaN(l) || l === null)
      return n;
    p ? w = l : f && f.length > 2 ? w = m(l) / (f.length - 2) : d !== b ? w = (l - b) / (d - b) : w = 1, w = P(w), p || (w = S(w)), C !== 1 && (w = Dn(w, C)), w = c[0] + w * (1 - c[0] - c[1]), w = Q(w, 0, 1);
    const L = Math.floor(w * 1e4);
    if (_ && g[L])
      k = g[L];
    else {
      if (R(a) === "array")
  ...
  return x.classes = function(l) {
    ...
    const p = a.length;
    if (l.length === p && b !== d)
      for (let k of Array.from(l))
        s.push((k - b) / (d - b));
    else {
      for (let k = 0; k < p; k++)
        s.push(k / (p - 1));
      if (l.length > 2) {
        const k = l.map((L, N) => N / (l.length - 1)), w = l.map((L) => (L - b) / (d - b));
        w.every((L, N) => k[N] === L) || (P = (L) => {
          if (L <= 0 || L >= 1) return L;
          let N = 0;
          for (; L >= w[N + 1]; ) N++;
          const v = (L - w[N]) / (w[N + 1] - w[N]);
          return k[N] + v * (k[N + 1] - k[N]);
        });
      }
    }
    return o = [b, d], x;
    ...
    return l == null && (l = !0), h = l, $(), h ? S = function(p) {
      const k = j(0, !0).lab()[0], w = j(1, !0).lab()[0], L = k > w;
      let N = j(p, !0).lab()[0];
      const v = k + (w - k) * p;
    ...
    if (arguments.length === 0)
      ...
      if (f && f.length > 2)
        for (let L = 1, N = f.length, v = 1 <= N; v ? L < N : L > N; v ? L++ : L--)
          w.push((f[L - 1] + f[L]) * 0.5);
      else
        w = o;
      k = w.map((L) => x(L));
    }
    return E[p] && (k = k.map((w) => w[p]())), k;
  }, x.cache = function(l) {
    return l != null ? (_ = l, x) : _;
  }, x.gamma = function(l) {
...
function br(e = 300, t = -1.5, n = 1, r = 1, o = [0, 1]) {
  ...
  const f = function(a) {
    const i = H * ((e + 120) / 360 + t * a), b = lr(o[0] + c * a, r), h = (s !== 0 ? n[0] + a * s : n) * b * (1 - b) / 2, g = ur(i), _ = ir(i), C = b + h * (-0.14861 * g + 1.78277 * _), y = b + h * (-0.29227 * g - 0.90649 * _), m = b + h * (1.97294 * g);
    return E(Ye([C * 255, y * 255, m * 255, 1]));
  };
  ...
  if (t.substr(0, 1) === "c" && (c.push(r), c.push(o)), t.substr(0, 1) === "e") {
    ...
    for (g.push(o); d; ) {
      ...
      for (let m = 0; m < n; m++)
        y[m] = null;
      for (let m = 0; m < a; m++)
        f = i[m], y[f] === null ? y[f] = s[m] : y[f] += s[m];
      for (let m = 0; m < n; m++)
        y[m] *= 1 / b[m];
      d = !1;
      for (let m = 0; m < n; m++)
    ...
    let C = [];
    for (let y = 0; y < n; y++)
      C.push(_[y][0]), C.push(_[y][_[y].length - 1]);
    C = C.sort((y, m) => y - m), c.push(C[0]);
    for (let y = 1; y < C.length; y += 2) {
      const m = C[y];
...
const { sqrt: W, pow: O, min: $r, max: xr, atan2: nt, abs: rt, cos: pe, sin: ot, exp: Lr, PI: st } = Math;
function Nr(e, t, n = 1, r = 1, o = 1) {
  ...
  e = new u(e), t = new u(t);
  const [f, a, i] = Array.from(e.lab()), [b, d, h] = Array.from(t.lab()), g = (f + b) / 2, _ = W(O(a, 2) + O(i, 2)), C = W(O(d, 2) + O(h, 2)), y = (_ + C) / 2, m = 0.5 * (1 - W(O(y, 7) / (O(y, 7) + O(25, 7)))), S = a * (1 + m), P = d * (1 + m), j = W(O(S, 2) + O(i, 2)), $ = W(O(P, 2) + O(h, 2)), x = (j + $) / 2, l = s(nt(i, S)), p = s(nt(h, P)), k = l >= 0 ? l : l + 360, w = p >= 0 ? p : p + 360, L = rt(k - w) > 180 ? (k + w + 360) / 2 : (k + w) / 2, N = 1 - 0.17 * pe(c(L - 30)) + 0.24 * pe(c(2 * L)) + 0.32 * pe(c(3 * L + 6)) - 0.2 * pe(c(4 * L - 63));
  let v = w - k;
  v = rt(v) <= 180 ? v : w <= k ? v + 360 : v - 360, v = 2 * W(j * $) * ot(c(v) / 2);
  const Z = b - f, le = $ - j, ue = 1 + 0.015 * O(g - 50, 2) / W(20 + O(g - 50, 2)), be = 1 + 0.045 * x, He = 1 + 0.015 * x * N, Yt = 30 * Lr(-O((L - 275) / 25, 2)), Xt = -(2 * W(O(x, 7) / (O(x, 7) + O(25, 7)))) * ot(2 * c(Yt)), qt = W(
    O(Z / (n * ue), 2) + O(le / (r * be), 2) + O(v / (o * He), 2) + Xt * (le / (r * be)) * (v / (o * He))
  );
  return xr(0, $r(100, qt));
}
...
const Or = (...e) => {
  ...
  return [a, i, b, ...o.length > 0 && o[0] < 1 ? [o[0]] : []];
}, U = /((?:-?\d+)|(?:-?\d+(?:\.\d+)?)%|none)/.source, q = /((?:-?(?:\d+(?:\.\d*)?|\.\d+)%?)|none)/.source, we = /((?:-?(?:\d+(?:\.\d*)?|\.\d+)%)|none)/.source, X = /\s*/.source, fe = /\s+/.source, We = /\s*,\s*/.source, _e = /((?:-?(?:\d+(?:\.\d*)?|\.\d+)(?:deg)?)|none)/.source, ae = /\s*(?:\/\s*((?:[01]|[01]?\.\d+)|\d+(?:\.\d+)?%))?/.source, Nt = new RegExp(
  "^rgba?\\(" + X + [U, U, U].join(fe) + ae + "\\)$"
), Et = new RegExp(
  "^rgb\\(" + X + [U, U, U].join(We) + X + "\\)$"
), Rt = new RegExp(
  "^rgba\\(" + X + [U, U, U, q].join(We) + X + "\\)$"
), Ct = new RegExp(
  "^hsla?\\(" + X + [_e, we, we].join(fe) + ae + "\\)$"
), vt = new RegExp(
  "^hsl?\\(" + X + [_e, we, we].join(We) + X + "\\)$"
), jt = /^hsla\(\s*(-?\d+(?:\.\d+)?),\s*(-?\d+(?:\.\d+)?)%\s*,\s*(-?\d+(?:\.\d+)?)%\s*,\s*([01]|[01]?\.\d+)\)$/, Pt = new RegExp(
  "^lab\\(" + X + [q, q, q].join(fe) + ae + "\\)$"
), St = new RegExp(
  "^lch\\(" + X + [q, q, _e].join(fe) + ae + "\\)$"
), Ot = new RegExp(
  "^oklab\\(" + X + [q, q, q].join(fe) + ae + "\\)$"
), Bt = new RegExp(
  "^oklch\\(" + X + [q, q, _e].join(fe) + ae + "\\)$"
...
const at = 42, Fr = [
  ...
  (n, r) => r / Math.max(n, 1)
) : (console.warn("expected prop to be 'number[]' or 'int'", e), []), ze = (e, t) => ((t ? 1 - e : e) * 100).toFixed(2) + "%", Se = (e, t, n = t, r = !1) => [ze(t), e(r ? 1 - n : n).hex()], F = (e, t = !1) => {
  if (e === "" || e === !0) return !0;
  if (!e) return;
  ...
  let {
    ...
    colorPadding: _,
    colorClass: C,
    centeredEdgeLabels: y
  } = e;
  t = F(t), n = F(n), r = F(r, !0), o = F(o, !0), _ = F(_, !0), s = F(s), f = F(f), b = F(b), g = F(g), n === !0 && (Array.isArray(r) ? n = [...r] : Array.isArray(o) && (n = [...o])), c ^ s && (Array.isArray(r) && (r = r.reverse().map((l) => 1 - l)), Array.isArray(o) && (o = o.reverse().map((l) => 1 - l)));
  ...
  let $;
  if (t === !0 ? $ = E.scale(Fr).domain(Hr) : t === void 0 ? t = ["#fff", "#000"] : Array.isArray(t) || (t = E.brewer[t] || ["#fff", t]), t !== !0 && c ^ s && (t = [...t].reverse()), $ = $ || E.scale(t), d && ($ = $.mode(d)), h && ($ = $.gamma(h)), g && ($ = $.correctLightness()), _ && ($ = $.padding(_)), o)
    switch (C) {
      case "average":
        $ = $.domain([0, 1]);
        break;
  ...
  if (a === "" && (a = $(s ? 1 : 0).hex()), i === "" && (i = $(s ? 0 : 1).hex()), !o || Number(o) <= 1)
    ...
    if (Array.isArray(l)) {
      ...
      S = l.reduce((k, w, L, N) => {
        let v = N[L + 1], Z = (w + v) / 2;
        return L < p && L && k.push(Se($, w, Z, b)), L < p - 1 && k.push(Se($, v, Z, b)), k;
      }, []), m = l;
    }
  }
  if (r !== void 0) {
  ...
  if (n !== void 0) {
    ...
    switch (m && n.length - m.length) {
      ...
      case -1:
        ...
        break;
      default:
        n.length > 1 ? (l = (p) => p / (n.length - 1), y = !1) : (l = () => 0.5, y = !0);
        break;
    }
    j = n.map((p, k) => [ze(s ? 1 - l(k) : l(k)), p]);
  }
  return {
    stops$: S,
    ticks$: P,
    ...
    after: i,
    vertical: s,
    klass: `dv-sb${a ? " b" : ""}${i ? " a" : ""}${f ? " f" : ""}${s ? " v" : " h"}${y ? " c" : ""}`
  };
};
let Ur = 0;
const Vr = Tt({
      ...
      return I("svg", {
        ...
        I("defs", [
          ...
          I(
            ...
            {
              id: `lg${t}`,
              gradientTransform: f ? "translate(-1,1) rotate(-90)" : null
            },
            n == null ? void 0 : n.map(
              (i) => I("stop", { offset: i[0], "stop-color": i[1] })
            )
          )
        ]),
  ...
  for (const [r, o] of t)
    n[r] = o;
  return n;
}, t0 = /* @__PURE__ */ Jr(Vr, [["__scopeId", "data-v-fada05ef"]]);
export {
  t0 as default
};
